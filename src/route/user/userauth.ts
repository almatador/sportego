...
interface User {
  ...
  cityId: number;
  countryId: number;
  status: 'ACTIVE' | 'SUSPENDED' | 'NEW'; // نوع التعداد (Enum) للحالة
  date_of_birth?: Date;
  weight?: number;
  height?: number;
  gender: 'MALE' | 'FEMALE'; // إضافة حقل للنوع
  profilePic?: string;
  createdAt?: Date;
  updatedAt?: Date;
  deletedAt?: Date;
...
userRoute.post('/authuser', async (req, res) => {
  const { name, email, password, confPassword, cityId, countryId, status, date_of_birth, weight, height ,gender } = req.body;
  try {
    ...
    const [result]: any = await connection.execute(
      'INSERT INTO user (name, email, password, confPassword, cityId, countryId, status,date_of_birth,weight,height, gender  ) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [name, email, password, confPassword, cityId, countryId, status, date_of_birth, weight, height,gender ]
    );
    const userId = result.insertId;

    const token = generateToken(userId); // استخدم الحالة كدور
    // إضافة مفتاح سري
    ...
    res.cookie('authToken', token, {
      ...
      secure: true,
      maxAge: 3600000,
    }); res.status(201).json({ id: userId, name, email, password, confPassword, cityId, countryId, status, date_of_birth, weight, height,gender , token });
  } catch (error) {
    console.error(error);
...
userRoute.post('/upload-profile-pic', upload.single('profilePic'), async (req, res) => {
  ...
  try {
    ...
    const [result]: any = connection.execute(
      'UPDATE user SET profile_image_path = ? WHERE id = ?',
...
userRoute.post('/login', async (req, res) => {
  ...
  connection.query(query, [email], async (err, results: mysql.RowDataPacket[]) => {
    ...
    const tokenQuery = 'SELECT * FROM secretkeyuser WHERE userId = ?';
...
userRoute.post('/logout', (req, res) => {
  ...
  const deleteTokenQuery = `DELETE FROM secretkeyuser WHERE token = ?`;
